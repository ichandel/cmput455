...
# ---------- A2: to be implemented by students ----------

    # To implement for your assignment:
    # We assume this function returns: winner, winning_move
    # winner is 1 or 2, this is not relative to the current player
    ...
    # We will test that you print unknown when given unsolvable positions (positions that are way too difficult for even the best solver implementations)
    # so don't guess at who might win if you can't actually solve the position.
    def solve(self, args):
        
        # Functions necessary for checking timeouts
        # This method for timeouts doesn't work on windows, you will get an error
        # Run on linux (you should be running this on the university lab machines anyways)
        ...
        try:
            ...
            signal.alarm(0)
            self.init_game([str(self.width), str(self.height), str(self.handicap), str(self.score_cutoff), self.board_str])
